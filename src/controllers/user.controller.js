...
export const applyForMentorship = async (req, res) => {
  try {
    ...
    const { designation, company, experience, skills, linkedin, reason } =
      req.body;
    ...
    return res
      .status(500)
      .json({ error: "Error applying for mentorship request" });
  }
};
...
export const updateProfile = async (req, res) => {
  try {
    ...
    if (bio && bio.length > 20) {
      return res
        .status(400)
        .json({ error: "Bio must be less than 20 characters" });
    }
...
export const getUserProfileData = async (req, res) => {
  try {
    ...
    const userBlogs = await BlogModel.find({ postedBy: userId }).sort({
      createdAt: -1,
    });

    const userProjects = await ProjectModel.find({ postedBy: userId }).sort({
      createdAt: -1,
    });

    const userIdeas = await IdeaModel.find({ postedBy: userId }).sort({
      createdAt: -1,
    });
...
export const updateUserInterests = async (req, res) => {
  ...
  if (!req.user) {
    return res.status(401).json("Login to save your interests!");
  }
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Interests updated", interests: user.interests });
  } catch (err) {
